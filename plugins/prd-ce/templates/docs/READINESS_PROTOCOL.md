...
Readiness scoring tells you — at three levels of granularity — whether work in a PRD-CE repo meets
the evidence floor for owner gate review. It never authorizes a PRD transition. This document is the
canonical schema reference. Direct installs include the discipline rule at
`.claude/rules/07-readiness-protocol.md`; plugin-native consumers receive the same methodology
guidance live from the plugin. For narrative context see the README's "Readiness Scoring" section.
...
The orchestrator `readiness.py` runs all three in dependency order (SoT → EPIC → stage) and produces a single text report + machine-readable JSON. Its direct-install path is `scripts/readiness.py`; plugin-native invocation uses `${CLAUDE_PLUGIN_ROOT}/scripts/readiness.py` with `--repo` pointing at the consumer repository.
...
> **Development Graph dimensions** read `status/devgraph.json` — see [`DEVELOPMENT_GRAPH.md`](DEVELOPMENT_GRAPH.md) for the schema and visualization contract. They are **additive** to the nine spec-phase dimensions: raw weights are relative and renormalized at runtime (§8), so adding two dormant dimensions changes **no** score for a repo that has not yet built anything. They activate only once a v0.7 build produces a dev graph.

### Stage dimensions

| Dimension | Weight | What it measures |
|---|---:|---|
| `required_ids_present` | 0.30 | Fraction of gate-mandated prefixes meeting their minimum count. Sourced from `GATE_REQUIREMENTS` in `_readiness/stage.py` (mirrors the methodology package's gate criteria). |
...
**Constants** (in `_readiness/common.py` beside the active readiness runtime):
...
**Universal defaults** live in `_readiness/common.py` beside the active readiness runtime. Per-gate overrides live in `PRD.md`'s `readiness_inputs.weight_overrides` for the stage in question. Overrides merge with defaults; renormalization ensures sum = 1.0.
...
Direct-install layout:

```bash
# Compute all three layers, print text report, exit with pass/warn/block code
python3 scripts/readiness.py run

# Print report from existing readiness.json without recomputing
python3 scripts/readiness.py status

# Emit raw JSON instead of text
python3 scripts/readiness.py run --json

# CI-friendly: suppress output, exit code only
python3 scripts/readiness.py run --quiet

# Override target gate
python3 scripts/readiness.py run --gate v0.7

# Run one layer at a time (for debugging)
python3 scripts/compute-sot-readiness.py all
python3 scripts/compute-readiness.py epics/EPIC-01.md --merge
python3 scripts/compute-prd-readiness.py --gate v0.8
```

Plugin-native layout (the consumer repository remains the scoring target):

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/readiness.py" run --repo "$PWD"
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/readiness.py" status --repo "$PWD"
```

If dependencies are missing, install them from the matching runtime; do not assume a
repository-local `scripts/` directory in plugin mode:

```bash
python3 -m pip install -r scripts/requirements.txt
python3 -m pip install -r "${CLAUDE_PLUGIN_ROOT}/scripts/requirements.txt"  # plugin mode
```
...
1. **Stage** tells you whether the readiness floor is met for owner gate review; it does not
   authorize a PRD transition. If BLOCK, the next question is *why*.
