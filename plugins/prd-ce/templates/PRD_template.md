...
- `CLAUDE.md` — operating instructions and documentation discipline; not product fact authority.
- `README.md` — repository orientation and current status.
- `PRD.md` — this file. Owns strategy and lifecycle authorization from v0.1 → v1.0.
- `SoT/` — accepted, durable product records referenced by this PRD.
- `epics/EPIC-{XX}-<slug>.md` — approved execution window created only at v0.7+.
- Gate criteria and lifecycle authorization live in this PRD; `README.md` reports current status.

Read in the order above. Product-truth precedence is `PRD.md` → accepted SoT → approved EPIC;
`CLAUDE.md` governs behavior rather than product facts.
...
| Field                      | Value                              |
| -------------------------- | ---------------------------------- |
| **Current Lifecycle Gate** | v0.1 Spark                         |
| **Last Updated**           | YYYY-MM-DD                         |
| **Last Editor**            | {Name / Agent}                     |
| **Status**                 | Discovery                          |
| **Next Target Gate**       | v0.2 Market Definition             |
| **Related EPIC**           | None (EPICs begin at v0.7)         |
| **SoT Snapshot**           | None yet                           |
...
> **Revision Pattern**: When looping back, add a new row (e.g., `v0.3r1`). Before v0.7,
> reference the current PRD gate record and accepted SoT IDs; at v0.7+, a revision may also
> reference the approved EPIC that triggered it.
...
> **Scoring**: Each RISK- maps to a scoring category (Market/User/Technical). Score = Impact × Likelihood × Status Weight. Use the scoring quick reference below.
