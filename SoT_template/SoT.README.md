...
> **Uninitialized seed:** content files ship with `template_state: uninitialized`. Their numbered
> blocks and HTML cards are format examples, not accepted product records; ID validation and
> readiness ignore them. Before accepting the first record in a file, replace its example content,
> set `template_state: active`, and add the real ID to the current PRD gate snapshot.
...
2. Replace the example content in the first SoT file you need; do not append beside fake examples
...
5. Cross-link IDs from the PRD gate snapshot and other active SoT files
