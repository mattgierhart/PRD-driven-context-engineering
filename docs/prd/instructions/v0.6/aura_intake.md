...
> File path retains the `aura_intake.md` naming convention for automation, but responsibility shifts to the Build Lead / architecture owner at this stage.
...
**Inputs:** Architecture Packets from the [PRD v0.6 Research Agent Instructions](./research_agents.md), plus unresolved risks/mitigations from v0.5.
...
4. **Prompt & Acceptance Readiness:** Are acceptance criteria and AI prompt scaffolds tied to SoT IDs so build agents avoid vague instructions (see [CFD-401](../../../../source_of_truth/customer_feedback.md#cfd-401-partial-ghm-adoption-during-48-hour-inventory-build))?
...
8. **Prompt & Device Prep:** Draft acceptance criteria, AI prompt skeletons, and device validation plans that cite the relevant SoT IDs so v0.7 execution starts with unambiguous specs (per [CFD-401](../../../../source_of_truth/customer_feedback.md#cfd-401-partial-ghm-adoption-during-48-hour-inventory-build)).
...
| **Prompt & Device Prep** | Acceptance criteria + device validation plan documented with SoT citations (see [CFD-401](../../../../source_of_truth/customer_feedback.md#cfd-401-partial-ghm-adoption-during-48-hour-inventory-build)). |

## Collaboration & Packaging
- Draft updates in `[architecture-option]__prd-v0.6.md` before merging to PRD.
- Update SoT directories (`source_of_truth/architecture/`, etc.) with new ID files and link them from README/PRD.
