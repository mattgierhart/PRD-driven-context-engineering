...
The root `README.md` embeds screenshots from [`docs/assets/sot-html/`](../../docs/assets/sot-html/).
They are generated example renders, not hand-made — and they live under `docs/assets/`, outside the
install allowlist, so the 1.3 MB of PNGs never ship with `/prd-ce:init` (the HTML review templates in
this directory do seed; their example renders do not). Whenever the pages change (new entries replacing
placeholders, style changes, a new page), regenerate them:
