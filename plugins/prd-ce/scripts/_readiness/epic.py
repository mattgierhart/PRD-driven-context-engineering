...
def parse_epic(epic_file: Path, inputs_override: Optional[dict]) -> EpicContext:
    ...
    filename_id = epic_id_from_filename(epic_file)
    if filename_id is None:
        raise SystemExit(f"error: malformed EPIC filename: {epic_file.name}")
    epic_id = inputs.get("target") or filename_id
...
def compute_dependency_readiness(ctx: EpicContext, repo: Path) -> tuple[float, list[dict]]:
    ...
    if not deps:
        return 100.0, []
    epic_files = {
        epic_id_from_filename(path): path
        for path in iter_epic_files(repo / "epics")
    }
    complete = 0
    unmet_items: list[dict] = []
    for dep in deps:
        dep_file = epic_files.get(dep)
        if dep_file is None:
...
def compute_file_readiness(ctx: EpicContext, index: dict[str, SoTEntry],
    ...
    for rel in files_used:
        ...
        if is_uninitialized_template(text) or (
            not headings and (has_placeholder or len(text.strip()) < 100)
        ):
            stubs.append(rel)
...
def compute(epic_file: Path, repo: Path, inputs_override: Optional[dict]) -> dict:
    ...
    ctx.referenced_ids = [
        entry_id for entry_id in ctx.referenced_ids
        if entry_id.split("-", 1)[0] in domain
    ]
