...
from .common import (
    canonical_graph_markdown_files,
    HEADING_DEF_RE,
    ID_RE,
...
def index_all_ids(repo: Path) -> dict[str, set[str]]:
    ...
    epics = repo / "epics"
    for p in iter_epic_files(epics):
        epic_id = epic_id_from_filename(p)
        if epic_id and "EPIC" in known_prefixes:
            out.setdefault("EPIC", set()).add(epic_id)
        ...
        for line in text.splitlines():
            ...
            if m:
                ...
                if prefix not in known_prefixes:
                    continue
                out.setdefault(prefix, set()).add(id_)
    return out
...
def compute_cross_ref_integrity(repo: Path, id_index: dict[str, set[str]],
    ...
    scan_files = canonical_graph_markdown_files(repo)
...
def compute_stage(gate: str, repo: Path, readiness_json: dict) -> dict:
    ...
    # The domain profile is the closed registry. A missing profile uses the canonical fallback;
    # an explicit consumer profile may add or remove prefixes without defaults leaking back in.
    known_prefixes = set(load_domain_profile(repo))
