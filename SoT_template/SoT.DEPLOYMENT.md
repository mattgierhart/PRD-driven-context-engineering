...
> **Status**: Uninitialized template — no deployment records are accepted
...
- `DEP-XXX` - {Deployment using this secret}
- `RUN-XXX` - {Runbook for rotation}
...
- `MON-XXX` - {Metrics to watch}
- `SEC-XXX` - {Secrets required}
...
- `MON-XXX` - {Alert triggering this runbook}
...
- `RUN-XXX` - {Runbook if threshold breached}
