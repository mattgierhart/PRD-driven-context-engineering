...
Hooks receive a JSON object on stdin. The framework consumes the common fields below; provider
versions may add fields, and `permission_mode` is not present on every event. Consult the linked
event schema before adding a new hook.
...
| `permission_mode` | Current permission mode when supplied by the event |
...
|------|-------|--------|---------|
| Context Validation | `SessionStart` | `context-validation.sh` | Inject lifecycle-aware authority read order |
| Context Density Gate | `UserPromptSubmit` | `context-density-gate.sh` | Assess epic/gate context readiness |
| Subagent Memory Load | `SubagentStart` | `subagent-memory-load.sh` | Inject consumer agent MEMORY.md, including for plugin-scoped agent names |
| Subagent Memory Save | `SubagentStop` | `subagent-memory-save.sh` | Request extraction once; stage memory on the follow-up stop; report drift |
...
Event hooks are Bash scripts requiring only standard macOS/Linux utilities for their core
behavior. Python is optional for date math in `context-validation.sh` and for the
`metrics_drift_check.py` helper invoked by `subagent-memory-save.sh`; both paths degrade gracefully
...
The first `subagent-memory-save.sh` pass returns extraction feedback. Claude then retries
`SubagentStop` with `stop_hook_active: true`; that follow-up pass emits no further feedback and
best-effort stages the consumer MEMORY.md. The human should commit it with the convention above.

## Supported Events

This template uses 6 events: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`,
`SubagentStart`, and `SubagentStop`. Claude Code's provider-wide event list evolves; use the
[current official reference](https://code.claude.com/docs/en/hooks#hook-events) rather than a
copied count or frozen list.
