...
Design principles (from a cross-project failure analysis):
...
Usage:
  ...
  Direct:      echo '["src/api/handler.py", "tests/test_auth.py"]' | \
               python3 .claude/hooks/cascade_checklist.py
  Plugin:      echo '["src/api/handler.py", "tests/test_auth.py"]' | \
               python3 "${CLAUDE_PLUGIN_ROOT}/hooks/cascade_checklist.py"
"""
import json
import re
import sys
...
def main():
    ...
    try:
        ...
        # Hook stdin is a pipe and is not seekable. Reuse the captured payload for the
        # documented line-separated fallback.
        file_paths = [line.strip() for line in raw.splitlines() if line.strip()]
