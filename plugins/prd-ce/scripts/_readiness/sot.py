...
def compute_cross_ref_density(
    entries: list[SoTEntry], known_prefixes: set[str],
) -> tuple[Optional[float], list[dict]]:
    if not entries:
        return None, []
    with_refs = [e for e in entries if e.outbound_refs(known_prefixes)]
    score = (len(with_refs) / len(entries)) * 100.0
    isolated = [e for e in entries if not e.outbound_refs(known_prefixes)]
    unmet = [
...
def build_epic_consumer_map(repo: Path, domain: dict) -> dict[str, list[str]]:
    ...
    for epic in iter_epic_files(epics_dir):
        ...
        epic_id = epic_id_from_filename(epic)
        if not epic_id:
            continue
...
def compute_file(sot_file: Path, repo: Path, index: dict[str, SoTEntry],
    ...
    s, u = compute_cross_ref_density(entries, known_prefixes)
...
def main_cli() -> int:
    ...
    for sot in targets:
        block = compute_file(sot, repo, index, all_refs, consumer_map, set(domain))
