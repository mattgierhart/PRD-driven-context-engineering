...
| 15c | **Key-moments question research + template design** (docs/v2/V2_KEY_MOMENTS.md §4) | L | Dedicated session: per moment — the key questions, the bespoke template design (Graphify pattern: designed visual + deterministic pull script + rebuild triggers, canon §2), the emitted-record shape, the pass/fail line in its pack; decide M8 promotion + the three candidate moments |
